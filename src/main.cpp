...
#if defined(NDEBUG)
# error "KrustyCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa3c26a6e846fd4411bb8842be6865d4f8b05b86c024bdc167a4c3335a9e62abc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KrustyCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KrustyCoin Signed Message:\n";
...
{
    // KrustyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 69420 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 69420); // KrustyCoin: 69.42k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6.9 * 24 * 60 * 60; // KrustyCoin: 6.9 days
static const int64 nTargetSpacing = 02.4 * 60; // KrustyCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KrustyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KrustyCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7002d68b553edc19e5fc93290c44d5b45d5f6cefa5b3ce36e373ee24b859c34d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KRST News 05/May/2021: The Day Bitcoin Falls and KrustyCoin Rises";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b8a804c25c54d37c84f39f3543e636534e1bc820be846ac953df9560cfbbfc7e874acbccd427efb7299ff3b9062cb0ed67897bff7555e4e892dcedf36937b37e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1620242069;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084533191;

        if (fTestNet)
        {
            block.nTime    = 1620269420;
            block.nNonce   = 385952146;
        }
...
//         }
        ...
        assert(block.hashMerkleRoot == uint256("0xed055f41dc22837422d94a26d44c95f3190ef5a18c9734649afc1afa99b4f223"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb5, 0xdf }; // KrustyCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KrustyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KrustyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KrustyCoinMiner(CWallet *pwallet)
{
    printf("KrustyCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("krustycoin-miner");
    ...
    try { loop {
        ...
        printf("Running KrustyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KrustyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KrustyCoinMiner, pwallet));
}
